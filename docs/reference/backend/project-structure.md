...
CRD type definitions for the `memcached.c5c3.io/v1alpha1` API group:

- **`groupversion_info.go`** - Declares `GroupVersion`, `SchemeBuilder`, and `AddToScheme` for scheme registration.
- **`memcached_types.go`** - Defines `MemcachedSpec`, `MemcachedStatus`, `Memcached`, and `MemcachedList` with kubebuilder markers (`+kubebuilder:object:root=true`, `+kubebuilder:subresource:status`). Spec and status are minimal stubs to be extended in subsequent features.
...
1. Registers `clientgoscheme` and `memcached/v1alpha1` on the runtime scheme
...
- **`memcached_controller.go`** - `MemcachedReconciler` struct embedding `client.Client` with a `Scheme` field. Includes RBAC markers for memcacheds (full CRUD), status, and finalizers. `SetupWithManager` registers watches for `Memcached` CRs. The `Reconcile` method is a stub returning empty `Result`.
- **`suite_test.go`** - Ginkgo/Gomega test suite bootstrapping envtest with CRD paths pointing to `config/crd/bases/`. Creates a test client with the v1alpha1 scheme registered.
- **`memcached_controller_test.go`** - Integration tests validating CR creation, reconciler setup, and empty reconcile behavior.
