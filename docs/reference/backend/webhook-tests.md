...
| `TestValidatePDB` (table-driven, 14 cases) | REQ-007 | minAvailable only (pass), maxUnavailable only (pass), percentage minAvailable (pass), both set (fail), neither set (fail), disabled (pass), nil PDB (pass), nil HA (pass), minAvailable < replicas (pass), minAvailable = replicas (fail), minAvailable > replicas (fail), percentage skips replicas check, nil replicas skips check, maxUnavailable integer (pass) |
...
| REQ-ID | Requirement | Unit Tests | Envtest | E2E |
|--------|------------|------------|---------|-----|
| REQ-001 | Replicas defaults to 1; zero/explicit preserved | `EmptySpec`, `ReplicasZeroPreserved`, `PreservesExplicitValues`, `FullySpecifiedCRUnchanged`, `Idempotent` | `minimal CR`, `fully specified CR` | `webhook-defaulting` |
| REQ-002 | Image defaults to memcached:1.6; custom preserved | `EmptySpec`, `PreservesExplicitValues`, `EmptyStringImagePreserved`, `FullySpecifiedCRUnchanged` | `minimal CR`, `fully specified CR` | `webhook-defaulting` |
| REQ-003 | Memcached config initialized and partial-filled | `NilMemcachedConfig`, `PartialMemcachedConfig`, `VerbosityZeroExplicit`, `ExtraArgsPreserved`, `FullySpecifiedCRUnchanged` | `minimal CR`, `fully specified CR` | `webhook-defaulting` |
| REQ-004 | Monitoring sub-fields defaulted when section non-nil | `MonitoringExporterImage`, `MonitoringExporterImagePreserved`, `NilMonitoringStaysNil`, `ServiceMonitorDefaults`, `ServiceMonitorPartialPreserved`, `NilServiceMonitorStaysNil`, `MonitoringDisabledStillDefaults`, `ServiceMonitorFullySpecifiedPreserved`, `IdempotentWithMonitoringAndHA` | `fully specified CR` | — |
| REQ-005 | HA antiAffinityPreset defaulted when section non-nil | `AntiAffinityPreset`, `AntiAffinityPresetHardPreserved`, `NilHAStaysNil`, `HAWithPDBStillDefaultsPreset`, `IdempotentWithMonitoringAndHA` | `fully specified CR` | — |
| REQ-006 | Memory limit >= maxMemoryMB + 32Mi overhead | `TestValidateMemoryLimit` (10 cases), `TestValidateMemoryLimit_ErrorMessage` | `rejects insufficient memory limit` | `reject-insufficient-memory-limit` |
| REQ-007 | PDB mutual exclusivity, neither-set, minAvailable < replicas | `TestValidatePDB` (14 cases), `TestValidatePDB_ErrorMessages` | `rejects PDB minAvailable >= replicas`, `rejects PDB mutual exclusivity` | `reject-pdb-mutual-exclusivity`, `reject-pdb-neither-set`, `reject-pdb-min-available-ge-replicas` |
| REQ-008 | SASL/TLS require secret references when enabled | `TestValidateSecuritySecretRefs` (10 cases), `TestValidateSecuritySecretRefs_ErrorMessages` | `rejects SASL without secret`, `rejects TLS without secret` | `reject-sasl-without-secret-ref`, `reject-tls-without-secret-ref` |
| REQ-009 | Graceful shutdown: terminationGrace > preStopDelay | `TestValidateGracefulShutdown` (7 cases), `TestValidateGracefulShutdown_ErrorMessage` | `rejects graceful shutdown timing violation` | `reject-graceful-shutdown-invalid-period` |
| REQ-010 | Error aggregation, StatusError format, delete bypass, update propagation | `MultipleErrorsCollected`, `FourSimultaneousViolations`, `StatusErrorFormat`, `PropagatesErrors`, `ValidToInvalid`, `ValidCRAccepted`, `DeleteAlwaysSucceeds`, `InvalidCRStillDeletes`, `FullyPopulatedValidCR` | `accepts valid CR`, `minimal CR passes`, `rejects update to invalid config` | — |
