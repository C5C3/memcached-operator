...
```bash
kubectl apply -f https://github.com/c5c3/memcached-operator/releases/download/v0.4.0/memcached.c5c3.io_memcacheds.yaml
```

Replace `v0.4.0` with the desired version. Available releases are listed at
...
Replace `0.4.0` with the desired chart version. Override default values with
...
```bash
kubectl apply -f https://github.com/c5c3/memcached-operator/releases/download/v0.4.0/install.yaml
```
...
```bash
make deploy IMG=ghcr.io/c5c3/memcached-operator:v0.4.0
```

Replace `v0.4.0` with the desired version tag. This builds the full manifest
...
```bash
kubectl delete -f https://github.com/c5c3/memcached-operator/releases/download/v0.4.0/install.yaml
```
