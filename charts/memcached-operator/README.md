...
```bash
helm install memcached-operator oci://ghcr.io/c5c3/charts/memcached-operator --version 0.3.0
```
...
```bash
helm pull oci://ghcr.io/c5c3/charts/memcached-operator --version 0.3.0
```

This downloads `memcached-operator-0.3.0.tgz` to your current directory. Extract and inspect with:

```bash
tar xzf memcached-operator-0.3.0.tgz
```
